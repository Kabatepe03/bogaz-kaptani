...
var engine_order := 0.0
var rudder_state := 0.0
var ground_speed_kn := 0.0
var drift_angle_deg := 0.0
var critical_roll_timer := 0.0
...
func _build_world() -> void:
	var real_world_resource := load(V12_WORLD_PATH)
	...
	var real_world := (real_world_resource as PackedScene).instantiate()
...
func _update_approach_markers() -> void:
	if approach_root == null:
		return
	for child in approach_root.get_children():
		child.queue_free()
	var target := eceabat_stop if route_forward else canakkale_stop
	var start := canakkale_stop if route_forward else eceabat_stop
	var inbound := (target - start).normalized()
	var starboard := Vector3.UP.cross(inbound).normalized()
	for distance in [320.0, 220.0, 135.0, 75.0]:
		var centre := target - inbound * distance
		for side in [-1.0, 1.0]:
			var marker := _make_approach_buoy(Color(0.95, 0.16, 0.08) if side < 0.0 else Color(0.08, 0.88, 0.26))
			marker.global_position = centre + starboard * side * 22.0 + Vector3(0, 0.35, 0)
...
func _update_dynamic_roll() -> void:
	...
	var weather_roll := sin(Time.get_ticks_msec() * 0.00165) * wind_strength * 0.68
	load_roll_deg = base_load_roll + weather_roll
	var roll_abs := absf(load_roll_deg)
	var comfort_loss := roll_abs * 0.010 + absf(steer * velocity_knots) * 0.005 + wind_strength * 0.012
...
func _update_ferry(delta: float) -> void:
	...
	var target_speed := engine_order * MAX_AHEAD_MPS if engine_order >= 0.0 else (-engine_order / 0.45) * -MAX_ASTERN_MPS
	var speed_error := target_speed - surge_mps
	var prop_gain := 0.105 + 0.055 * absf(engine_order)
	var prop_accel := clampf(speed_error * prop_gain, -0.22, 0.16)
	surge_mps += prop_accel * delta

	# Longitudinal resistance grows approximately with v²; low-speed coasting remains visible.
	var resistance := 0.0032 * surge_mps * absf(surge_mps)
	...
	var lateral_damping := 0.23 + absf(surge_mps) * 0.095
	...
	var prop_wash := absf(engine_order) * 1.35
	var water_flow := absf(surge_mps) + prop_wash
	var rudder_authority := clampf((water_flow * water_flow) / 28.0, 0.0, 1.0)
	var desired_yaw_rate := rudder_state * direction_sign * deg_to_rad(1.48) * rudder_authority
	var yaw_accel := deg_to_rad(0.22 + water_flow * 0.075)
	...
	var c_dock := GeoReference.to_local(GeoReference.CANAKKALE_DOCK)
	var e_dock := GeoReference.to_local(GeoReference.ECEABAT_DOCK)
	var channel_mid := (c_dock + e_dock) * 0.5
	var dist_mid := ferry.global_position.distance_to(channel_mid)
	var central_factor := clampf(1.0 - dist_mid / 3200.0, 0.28, 1.0)
	var south_surface := Vector3(-0.10, 0.0, 0.995).normalized()
	var weather_current_factor := clampf(0.78 + current_strength * 0.55, 0.72, 1.35)
	var current_speed := (0.43 + 0.42 * central_factor) * weather_current_factor
	var current_velocity := south_surface * current_speed
	var wind_velocity := wind_vector * (wind_strength * 0.052)

	var through_water := forward * surge_mps + starboard * sway_mps
	var ground_velocity := through_water + current_velocity + wind_velocity
	...
	var turn_heel := rad_to_deg(yaw_rate) * surge_mps * 0.24
	var drift_heel := sway_mps * 0.72
	var wave_roll := sin(Time.get_ticks_msec() * 0.00122) * (0.24 + wind_strength * 0.12)
	var target_roll := deg_to_rad(base_load_roll + turn_heel + drift_heel + wave_roll)
	ferry.rotation.z = lerp_angle(ferry.rotation.z, target_roll, clampf(delta * 0.92, 0.0, 1.0))
	var pitch_wave := sin(Time.get_ticks_msec() * 0.00088) * (0.16 + wind_strength * 0.055)
	var accel_pitch := clampf(prop_accel * -1.6, -0.35, 0.35)
