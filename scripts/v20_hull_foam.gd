...
func _boot() -> void:
	...
	var bow_mesh: MeshInstance3D = MeshInstance3D.new()
	...
	var side_mesh: MeshInstance3D = MeshInstance3D.new()
...
func _process(delta: float) -> void:
	...
	var instant_speed: float = p.distance_to(last_position) / maxf(delta, 0.001)
	...
	if foam_material != null:
		var intensity: float = clampf((filtered_speed - 0.35) / 5.8, 0.05, 0.86)
		foam_material.set_shader_parameter("intensity", intensity)
...
func _build_bow_wave_mesh() -> ArrayMesh:
	...
	for side: float in sides:
		...
		for i in range(segments + 1):
			...
			var z: float = -39.0 + t * 28.0
			var center_x: float = side * (0.8 + float(pow(t, 0.62)) * 10.7)
			var width: float = 0.65 + t * 1.85
			vertices.append(Vector3(center_x - side * width * 0.5, 0.0, z))
			vertices.append(Vector3(center_x + side * width * 0.5, 0.0, z))
			uvs.append(Vector2(0.0, t))
			uvs.append(Vector2(1.0, t))
			var alpha: float = 1.0 - t * 0.38
...
func _build_side_wash_mesh() -> ArrayMesh:
	...
	for side: float in sides:
		...
		for i in range(segments + 1):
			var t: float = float(i) / float(segments)
			var z: float = -29.0 + t * 58.0
			var hull_x: float = side * (8.35 - absf(z) / 44.0 * 0.75)
			vertices.append(Vector3(hull_x, 0.0, z))
			vertices.append(Vector3(hull_x + side * 1.35, 0.0, z))
			uvs.append(Vector2(0.0, t))
			uvs.append(Vector2(1.0, t))
			var end_fade: float = smoothstep(0.0, 0.10, t) * (1.0 - smoothstep(0.90, 1.0, t))
			colors.append(Color(1.0, 1.0, 1.0, end_fade * 0.82))
			colors.append(Color(1.0, 1.0, 1.0, end_fade * 0.50))
		for i in range(segments):
			var a: int = start + i * 2
			indices.append_array(PackedInt32Array([a, a + 2, a + 1, a + 1, a + 2, a + 3]))
	return _mesh_from_arrays(vertices, uvs, colors, indices)
